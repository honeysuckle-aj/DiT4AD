...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        p_bar = tqdm(enumerate(loader), desc="Training", unit="batch")
        for i, (img, mask_img, mask) in p_bar:
            img = img.to(device)
            ...
            t = torch.LongTensor(np.ones(img.shape[0]) * np.random.randint(0, diffusion.num_timesteps)).to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data-path", type=str, default="dataset/capsule")
